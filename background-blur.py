...
while (True):
    ...
    prev_time = time.time()
    output_image = cv2.resize(output_image, (405, 540))
    cv2.imshow("Selfie Segmentation 123", output_image)
    ...
    if  keyb == 27:
        ...
        bg_image = cv2.imread('3.jpg')
    elif keyb == ord('2'):
        bg_image = cv2.imread('s.jpg')
    elif keyb == ord('3'):
        bg_image = cv2.imread('8.jpg')
    elif keyb == ord('b'):
        blur +=1
cap.release()
